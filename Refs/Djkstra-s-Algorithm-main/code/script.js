...
// Adds up the edges when a user clicks the button ADD Edges
const addEdges = () => {
  if (cnt < 2) {
...
// array to store temporary clicked elements to make an edge between the(max size =2)
let arr = [];
...
const appendBlock = (x, y) => {
  ...
  // Event for node on a click
  block.addEventListener("click", (e) => {
    // To avoide node overlaps
    e.stopPropagation() || (window.event.cancelBubble = "true");

    //  can't start adding edges if variable addEdge is false
    if (!addEdge) return;
...
// Node creation only when click event occurs on screen
...
// Draws a line/ edge between any of the two selected nodes.
const drawLine = (x1, y1, x2, y2, ar) => {
  // Avoids multiples edges from same two nodes
...
// Function to get coordinates (x, y) of clicked node
...
// Shortest path function...(Algorithm)
const findShortestPath = (el) => {
  ...
  // Array consiting of the cost of nodes
  let cost = [];
  for (i = 0; i < cnt; i++) {
    i === source
      ? null
  ...
  // Array consiting of the final cost of node
  let minCost = [];
  minCost[source] = 0;

  // Repeat the same till every node is marked visited
...
// Dislpay of Algorithmic steps animation-- sliding from right to left
